...
class ClusterPlotter(PhenotypePlotter):
    ...
    def generate_labels(self, exemplar, members):
        ...
        for c in members.columns:
            ...
            impact = f'{ex} [{lo}, {hi}]'
            desc = self.get_stripped_label_description(c, 36)
            labels.append(f'{desc} ({impact})')
        return labels
    ...
    def extract_cluster(self, phenotypes, cluster_data, n):
        ...
        channels = self.top_channels(exemplar)
    ...
    def plot_clusters(self, clustering, clusters,
        ...
        with PdfPages(file_name) as pdf:
            for datum in cluster_structures:
                ...
                title = (f'{project_name} Source {exemplar.name} '
                         f'(m={len(members.index)}, s={datum.mean:.4f})')

                self.plot_single_cluster(exemplar, members,
                                         expressions[datum.center],
                                         title)
                pdf.savefig()
                plt.close()
